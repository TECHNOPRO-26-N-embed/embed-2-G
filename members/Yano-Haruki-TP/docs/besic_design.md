...
|:--|:--|
| 作品タイトル | 温度が見えるモニター |
| 一言説明（何ができるガジェットか） | 室温を表示するモニター |
| 使用する共通部品（グループ共通） | 4Digit 7-Segment Display |
| 追加部品（個人・主なもの） | DHT11温度センサー、LED（赤・青）、抵抗、ブレッドボード |
| 必須機能の数（3-1の件数） | 3件 |
| 追加機能の数（3-2の件数） | 2件（ブザー警告、湿度表示） |
...
| 共通部品が担う役割 | DHT11温度センサーで測定した温度を4桁7セグメントで主表示（SS.hh形式、例：25.37）する |
| 接続するArduinoピン（予定） | 74HC595: D11(SER), D12(RCLK), D13(SRCLK) / 桁選択: D5-D8 |
| 個人の設計と共通部品が協調する箇所 | ボタンや温度センサ入力で状態を切替え、温度表示・警告表示を更新する |
| グループ内で統一すべき仕様 | 7セグの配線順（a-g,dp）、桁選択ピン、温度フォーマット（SS.hh）、警告閾値（例：28℃/20℃） |
...
Button（押下/長押し） ─────────────→┐                                       ┌──→ 4 Digit-7 Segment（温度表示SS.hh）
DHT11温度センサ ───────────────→┤  状態管理・温度測定・入力判定・表示多重化  ├──→ LED（高温:赤/低温:青）
                                      └───────────────────────────────────────┘   └──→ ブザー（高温/低温時: 任意）
```

補足説明：
- Buttonは温度表示のリセットや閾値切替トリガとして扱う。
- DHT11温度センサは定期的に温度を取得し、28℃超で赤LED点灯、20℃未満で青LED点灯、閾値超過時はブザーも鳴動（任意機能）。
- 表示更新と入力監視は全てmillis()ベースで並行処理し、delay()は使用しない。
...
|  |  |  |
|  |  |  |
| 温度監視中 | 温度を5分ごとに測定・表示 | LED消灯 |
| 高温警告 | 温度が28℃以上 | 赤LED点灯、ブザー鳴動（任意） |
| 低温警告 | 温度が20℃以下 | 青LED点灯、ブザー鳴動（任意） |
...
|  |  |  |  |  |  |
|  |  |  |  |  |  |
| currentState | 現在の状態 | int | 2B | 0 | 0:監視中, 1:高温, 2:低温 |
| temperature | 温度値 | float | 4B | 0.0 | 5分ごとに更新 |
| humidity | 湿度値 | float | 4B | 0.0 | 任意機能 |
| lastUpdateMillis | 更新タイマー | unsigned long | 4B | 0 | |
...
| — | メイン制御 | loop() | 状態に応じて各関数呼び出し | なし | なし | 常時 |
| F01 | 温度測定 | readTemperature() | DHT11から温度取得 | なし | float | loop() |
| F02 | 温度表示 | displayTemperature() | 7セグに温度表示 | float | なし | loop() |
| F03 | LED警告 | updateLED() | 温度閾値でLED制御 | float | なし | loop() |
| A01 | ブザー警告 | updateBuzzer() | 閾値でブザー制御 | float | なし | loop() |
| A02 | 湿度表示 | displayHumidity() | 7セグに湿度表示 | float | なし | loop() |
...
|  |  |  |  |
|  |  |  |  |
| 温度測定・表示 | 5分 | millis | 他処理と並行 |
| LED制御 | 常時 | millis | 状態に応じて即時反応 |
| ブザー制御 | 常時 | millis | 状態に応じて即時反応 |
...
|  |  |  |  |  |
|  |  |  |  |  |
| DHT11温度センサー | DHT11 | D2 | 入力 | 温度・湿度取得 |
| 7セグメントディスプレイ | 4Digit 7-Segment | D10, D9, D8 | 出力 | 温度/湿度表示 |
| LED（赤） | 5mm LED | D3 | 出力 | 高温警告 |
| LED（青） | 5mm LED | D4 | 出力 | 低温警告 |
| ブザー | パッシブブザー | D5 | 出力 | 任意機能 |
| 抵抗 | 220Ω | - | - | LED保護 |
| ブレッドボード | - | - | - | 回路組立用 |
...
**ピン競合チェック結果：** 競合なし / 競合あり → （内容）ディスプレイ10,9,8 温度センサー　2
...
**AIの回答（要約）：**
- チャタリング対策：ボタン入力にデバウンス処理が設計されている。
- delay()：主要な処理はmillis()で管理されており、delay()による問題は発生しない。
- ノイズ対策：アナログ入力のノイズ対策として閾値にバッファを設けている。
...
**AIの回答（要約）：**
- 必須機能（温度測定・表示・LED警告）は全て設計・テスト計画に反映されている。
- 追加機能（ブザー・湿度表示）も設計に含まれている。
- テストケースも異常系を含めて網羅されている。
...
| No | 指摘内容 | 指摘者 | 対応 |
|:---|:---|:---|:---|
| 1 | なぜ5分起きに表示が切り替わるのか |  コミョングンさん| 室温はそこまで変わりはしないと思うので5分起きでいいと思います|
| 2 |20~28の間にLEDを使ってもいいと思う  | 佐藤さん | 作業時間に余裕があれば作成したいと思います |
| 3 |  |  |  |
