...
| 共通部品が担う役割 | 温度センサーで測った温度を表示する |
| 接続するArduinoピン（予定） | 10, 9, 8（例） |
| 個人の設計と共通部品が協調する箇所 | 温度を測定し、閾値でLED点灯・ブザー鳴動 |
| グループ内で統一すべき仕様 | ピン番号、温度閾値の単位（℃） |
...
【入力】         【Arduino UNO R3】         【出力】
DHT11温度センサー ──→  温度取得・判定  ──→  7セグ表示
                                      └─→  LED（赤・青）
                                      └─→  ブザー（任意）

補足：温度が28℃以上で赤LED、20℃以下で青LEDを点灯。閾値超過時にブザー鳴動（任意）。
補足説明（図で表現しきれない場合）：
