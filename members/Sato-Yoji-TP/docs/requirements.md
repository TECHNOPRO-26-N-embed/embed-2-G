...
**一言説明：**センサでタイム測定するストップウォッチ

### 1-3. なぜこれを作るのか（動機・背景）
> ※ 解決したい不便さ・あったら嬉しい理由を書いてください。

**動機：**タイムを争う競技（陸上、水泳等）ボタンだと個人差が出てしまうが、センサでその差を限りなくなくすことができると考えたため。その簡易版として、ボタンを押せない状況でもスタート/ストップができると便利であると考えたため。念のため、ボタンでも動くようにはしたい。
