...
|:--|:--|
| 作品タイトル |手持無沙汰解消スロット |
| 一言説明（何ができるガジェットか） |スティックを- Y方向に倒すと数字が高速で回り、各停止ボタンで対応するリールを止められる |
...
| 共通部品が担う役割 | 数字を表示する |
| 接続するArduinoピン（予定） | D9（CLK）, D10（DIO） |
...
```
【入力】                 【Arduino UNO R3】            【出力】
スティック（Y軸） ──→┐                         ┌──→ 4Digit 7-Segment Display
                      ├── 状態管理・判定処理 ───┤
ボタン ───────────→┘                         └──→ LEDライト
```

補足説明（図で表現しきれない場合）：
- スティックY軸を- Y方向へ倒して抽選を開始し、ボタンでリールを停止。
...
[電源ON / 初期化]
  ↓（初期化完了）
[待機中] ──（スティックY <= 800 で開始・当選決定）──→ [抽選中] ──（ボタン押下：当選→7、非当選→ランダム数字表示）──→ [リール停止中]
   ↑                                                                 │
  └────────────────────（3リール停止＋結果演出完了）──────────────────────┘
```
...
| 抽選中 | スティックY軸が閾値以下になった瞬間に当選結果を内部で決定し、リールの数字を高速で回転表示 | 7セグメントディスプレイが数字を回転表示、LEDは順送り点灯 |
...
|:--|:--|:--|:--|:--|:--|
| isWinning | 当選フラグ（true=当選） | bool | 1B | false | 開始入力成立時に1/30で決定 |
| stickYValue | スティックY軸の入力値（0〜1023） | int | 2B | 512 | analogRead(A0)で取得し、<=800で開始 |
...
| LED順送り演出 | 200ms | **millis** | LED1→2→3を順番に点灯しつつ他処理を並行するため |
...
|:--|:--|:--|:--|:--|
| スティック | ジョイスティックY軸 | A0 | 入力 | アナログ入力（閾値800以下で開始） |
...
| スティック入力 | A0でアナログ値を取得（ジョイスティックの可変抵抗出力を使用） |
...
|:---|:---|:---|:---|:---|
| 1 | 抽選開始 | `handleStartInput()` | A0（スティックY軸） | テスト#1 |
